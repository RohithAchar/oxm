...
import { BadgeCheck, MapPin, MessageCircle, Share2, Heart, Phone, ChevronDown, ChevronUp } from "lucide-react";
...
export default function ProductViewV2Client({
  ...
  const handleShare = async () => {
    ...
    const shareText = getShareText(product.name, `₹${unitPrice?.toFixed ? unitPrice.toFixed(2) : unitPrice}`);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="lg:hidden">
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="absolute top-16 right-4 flex flex-col gap-2">
                ...
                >
                  <Heart className={`w-5 h-5 text-white transition-colors duration-200 ${isFavorite(product.id) ? 'fill-red-500 text-red-500' : ''}`} />
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
          ...
          <div className="p-4 space-y-4">
            <div>
              <h1 className="text-base font-semibold mb-2 leading-tight">{product.name}</h1>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {product.product_colors.map((c: Color) => (
                        ...
                        >
                          <span className="w-4 h-4 rounded-full" style={{ backgroundColor: (c as any).hex_code }} />
                          {c.name}
                        </button>
                      ))}
                    </div>
            ...
            {product.product_tier_pricing?.length > 0 && (
              <div className="bg-muted/50 rounded-lg p-4">
                <h3 className="font-medium mb-3">Pricing Tiers</h3>
                <div className="flex gap-2 overflow-x-auto no-scrollbar">
                  {product.product_tier_pricing
                    .slice()
                    .sort((a: any, b: any) => a.quantity - b.quantity)
                    .map((tier: any, idx: number) => (
                      <div key={tier.id ?? idx} className={`rounded-lg border px-3 py-2 text-center min-w-[6rem] ${
                        ...
                        <div className="font-semibold text-sm">₹{tier.price}</div>
                      </div>
                    ))}
                </div>
              </div>
            ...
            {product.product_specifications?.length > 0 && (
              <div className="bg-muted/50 rounded-lg p-4">
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${showSpecs ? 'rotate-180' : ''}`} />
                </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-10">
          ...
          <div>
            ...
            <div className="mt-6 sm:mt-8 bg-muted/50 rounded-lg p-4 sm:p-6 border">
              <h1 className="text-sm sm:text-base font-semibold mb-1 tracking-tight leading-snug">{product.name}</h1>
              <Description text={product.description} />
      ...
      <div className="lg:hidden fixed inset-x-0 bottom-0 z-[60] bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 border-t">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex-1">
            <div className="text-xs text-muted-foreground">Total • Qty {sampleQuantity}</div>
            <div className="text-lg font-bold">₹{totalPrice.toFixed(2)}</div>
          </div>
          <Button
            size="lg"
            className="rounded-full px-8 py-3 text-base font-semibold bg-primary hover:bg-primary/90 active:bg-primary/80 transition-all duration-200 active:scale-95 shadow-lg"
            onClick={() => {
